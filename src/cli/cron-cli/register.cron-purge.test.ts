...
async function writeJobs(storePath: string, jobs: Array<Record<string, unknown>>): Promise<void> {
  await saveCronStore(storePath, { version: 1, jobs: jobs as unknown as CronJob[] });
}
...
describe("cron purge", () => {
  ...
  it("--zombies (live) removes old files and creates no archive when jobs are unchanged", async () => {
    if (!tmp) {
      throw new Error("tmp missing");
    }
    await writeJobs(tmp.storePath, []);
  ...
  it("--expired removes one-shot jobs older than 7 days and archives jobs.json", async () => {
    ...
    const reloaded = await loadCronStore(tmp.storePath);
    ...
    const archiveContents = JSON.parse(
      fs.readFileSync(path.join(tmp.dir, archive), "utf-8"),
    ) as { jobs: { id: string }[] };
  ...
  it("--expired skips at-jobs that are currently running", async () => {
    ...
    const expiredFireAt = NOW - 8 * 86_400_000;
    // Build the job but we cannot write state.runningAtMs via buildAtJob because
    // saveCronStore maps state separately; use the store's state_json column path.
    // The simplest approach: write the job normally and then update state via saveCronStore.
    const job = {
      ...buildAtJob("running-old", expiredFireAt, "stuck"),
      state: { runningAtMs: NOW - 60_000 },
    };
    await writeJobs(tmp.storePath, [job]);
    ...
    const reloaded = await loadCronStore(tmp.storePath);
    expect(reloaded.jobs.map((j) => j.id)).toEqual(["running-old"]);
  });
  ...
  it("--all --dry-run lists zombies and expired jobs together", async () => {
    ...
    const expiredFireAt = NOW - 8 * 86_400_000;
    await writeJobs(tmp.storePath, [buildAtJob("expired-2", expiredFireAt, "old")]);
  ...
  it("orphaned/stale-running/duplicates report deferred and remove nothing", async () => {
    ...
    await writeJobs(tmp.storePath, [buildAtJob("future-1", NOW + 86_400_000, "soon")]);
    const oldFile = path.join(tmp.runsDir, "zz.jsonl");
    touchOld(oldFile, 30 * 86_400_000, NOW);
    ...
    expect(fs.existsSync(oldFile)).toBe(true);
    const reloaded = await loadCronStore(tmp.storePath);
    expect(reloaded.jobs.map((j) => j.id)).toContain("future-1");
    const archives = fs.readdirSync(tmp.dir).filter((name) => name.includes(".purged-"));
    expect(archives).toHaveLength(0);
    expect(report.classifiers.orphaned?.status).toBe("deferred");
    expect(report.classifiers.staleRunning?.status).toBe("deferred");
