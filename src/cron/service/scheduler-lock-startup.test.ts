...
import { acquireSchedulerLock, type SchedulerLockHandle } from "../scheduler-lock.js";
import { start } from "./ops.js";
  ...
  it("second start goes silent and sets schedulerLockHeld", async () => {
    ...
    try {
      ...
      const state = createCronServiceState({
        ...
        cronEnabled: true,
        log: noopLog,
        // null disables the global lock so this test controls lock acquisition via spy
        schedulerLockPath: lockPath,
      ...
      const acquireSpy = vi
        .spyOn(await import("../scheduler-lock.js"), "acquireSchedulerLock")
